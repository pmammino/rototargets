...
def get_targets():
    ...
    pitching = stats[["IP", "ERA", "WHIP", "W", "K"]].quantile(.97)
    SV = stats[["SV"]].quantile(1-(((9*12)*.2)/len(stats.index)))
    x = 1-(((9*15)*.999)/len(stats.index))
    ...
    stats_hitters_top = stats_hitters.head(14*16)
